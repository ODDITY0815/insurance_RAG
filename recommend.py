...
INTEREST_TAG_MAP = {
    "MZ_사회초년생_Pick": {
        "대상": ["#사회초년생", "#2030세대", "#1인가구", "#독립준비생"],
        "걱정": ["#생활비부족", "#예기치못한상해", "#전세사기걱정", "#우울증/마음건강"],
        "우선순위": ["#가성비_보험료", "#세액공제효과", "#스마트폰가입", "#유연한납입"],
        "변화": ["#첫출근", "#독립", "#연봉협상"]
    },
    "골든에이지_시니어케어": {
        "대상": ["#부모님", "#고령자", "#은퇴준비자", "#활동적시니어"],
        "걱정": ["#간병인사용", "#치매케어", "#낙상/골절", "#대상포진/질병"],
        "우선순위": ["#간편가입(유병자)", "#정기적건강검진", "#보장한도확대", "#가족통합보장"],
        "변화": ["#환갑/칠순", "#손주탄생", "#퇴직"]
    },
    "DINK_맞벌이부부_자산관리": {
        "대상": ["#맞벌이부부", "#무자녀가정", "#고소득자", "#부부통합관리"],
        "걱정": ["#소득단절리스크", "#노후생활비부족", "#자산손실", "#암/중증질환"],
        "우선순위": ["#연금액_지급", "#복리효과", "#종합자산보장", "#세제혜택최대화"],
        "변화": ["#결혼기념일", "#부동산매입", "#해외장기체류"]
    },
    "세상_모든_집사_모여라": {
        "대상": ["#반려견", "#반려묘", "#노령펫", "#다펫가정"],
        "걱정": ["#슬개골/관절염", "#피부질환", "#구강케어", "#배상책임(물림사고)"],
        "우선순위": ["#다빈도질병보상", "#보험료할인특약", "#실시간청구", "#예방접종지원"],
        "변화": ["#반려동물입양", "#노령기진입", "#이사"]
    },
    "초보부터_베테랑까지_드라이빙": {
        "대상": ["#초보운전자", "#고령운전자", "#가족운전자", "#업무용운전자"],
        "걱정": ["#교통사고처리지원금", "#벌금/변호사비용", "#보복운전피해", "#자동차사고상해"],
        "우선순위": ["#안전운전UBI할인", "#커넥티드카할인", "#신속한긴급출동", "#민사/형사합의금"],
        "변화": ["#신차출고", "#중고차구매", "#면허취득"]
    },
    "성장기_아이를_위한_든든함": {
        "대상": ["#태아/산모", "#영유아", "#초등학생", "#다자녀가정"],
        "걱정": ["#선천이상", "#성장기질병(호흡기)", "#학교/학원사고", "#아토피/피부질환"],
        "우선순위": ["#성장단계별보장", "#교육자금마련", "#납입면제", "#어린이전용서비스"],
        "변화": ["#출산예정", "#유치원/학교입학", "#이사"]
    },
    "주말_레저_프로취미러": {
        "대상": ["#골프매니아", "#등산/캠핑족", "#해외여행족", "#레저활동여행자"],
        "걱정": ["#홀인원축하금", "#장비도난/파손", "#산행중사고", "#휴대품손해"],
        "우선순위": ["#하루단기가입", "#종합레저보장", "#실속형보험료", "#간편한청구"],
        "변화": ["#시즌오픈", "#대회참가", "#해외출국"]
    },
    "사장님_사업장_안심케어": {
        "대상": ["#소상공인", "#카페/음식점운영", "#다중이용시설관리자", "#창업준비생"],
        "걱정": ["#화재손해", "#시설배상책임", "#음식물사고", "#영업중단손실"],
...
def get_product_by_tags(selected_tags: Dict[str, List[str]]) -> Optional[str]:
    ...
    for p_name, p_data in p_tags_db.items():
        ...
        risk_match = len(set(selected_tags.get("걱정", [])) & set(p_data.get("tags", {}).get("걱정", [])))
